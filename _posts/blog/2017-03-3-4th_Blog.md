...
To be honest, I am still far away from being an expert in the Recurrent Neural Networks but I did learn a lot of the fundamentals, ideas and implemented techinuqes regarding RNN. Not to mention the useful tips I found useful online during the project time. Therefore, I will spend some time writing down them for either a record for my future work or for other people's easy reference.  

# Preparation  
##1. Raw Data

I took a month of Reddit data and it is already 35G decompressed. What I did originally is to split the dataset into few seperate CSV file, find the question and answer pair with a filter of max length 20 and min length 3. I wanted a smaller dataset to quickly come up with some good results but if I had more time I would try the model on the complete dataset. However, long sentences are always issues for training in terms of training time and complexity.   
...
[Tensorflow Example Library](https://github.com/tensorflow/models) is a very active and complete resource for most advanced models in the industry(from papers published in last year or even this year). **Most importantly, Neural Networks take long time to train, sometimes up to months. It makes you feel much saver to work with some models that you are certain will work (All TF model examples have approved working model weights and published papers)!**

It worthes to mention that Keras has a very easy-to-implemented and powerful model on Image recorgnition, especially with the pretrained models. With some tasks, you can easily achieve 98% accuracy! *I would be very interested to see how Keras works in Image Captioning in future works.*

Last but not least, there are many useful tools in Keras you may want to use!(You will see a lot of examples below)
...
[!RNN Model](./assets/images/RNN model.png)
